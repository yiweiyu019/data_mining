...
if size>15000:
    sampleall = data.takeSample(False,15000)
else:
    sampleall = data.takeSample(False,int(size*0.8))

sample = sc.parallelize(sampleall)
initial_center = sample.takeSample(False,5*n_cluster)
initial_centero = []
...
#dis = [('4',[rid1,rid2...]),...]
all_cluster = sample.map(lambda x:computet(x,central)).groupByKey().mapValues(list)
ds_first = all_cluster.collectAsMap()
print('first')
ds_first = sorted(ds_first.items(),key = lambda x:len(x[1]),reverse=True)[:n_cluster]
ds_first2 = []
nnd = 0

for i in ds_first:
    ds_first2.append((str(nnd),initial_center[int(i[0])][1]))
    nnd += 1
central = ds_first2
for i in range(0,1):
    sample_cluster = sample.map(lambda x:computet(x,central)).groupByKey().mapValues(list)#.filter(lambda x:len(x[1])<=10)
    #ds_d1 = sample_cluster.collectAsMap()
    #ds_da = ds_d1
#for i in ds_d1:
#    ds_da[i].extend(ds_d1[i])
#summarization = [('4',({0:-39,1:83...},{0:160,1:100},3180)]
    summarization = sample_cluster.map(lambda x:compt_stat2(x)).map(lambda x:(x[0],list((x[1][0]).values()))).collect()
    central = summarization
summarization = sample_cluster.map(lambda x:compt_stat2(x)).collectAsMap()
print(DS_data.keys())
ds_n = 0
ds_da = sample_cluster.collectAsMap()
# for i in ds_da:
#     ds_n += len(ds_da[i])
#
# print(ds_n)
...
for i in range(0,len(initial_center2)):
    ...
    initial_info.append((i,initial_center2[i][1]))
...
for i in range(0,3):
    dis2 = data.map(lambda x:computet(x,initial_info)).groupByKey().mapValues(list)
    #ds_d1 = dis2.collectAsMap()
    summarization2 = dis2.map(lambda x: compt_stat2(x)).map(lambda x: (x[0], list((x[1][0]).values()))).collect()
    initial_info = summarization2
#more than one data point in cluster ---cs
CS_datat = dis2.filter(lambda x:len(x[1])>1).collectAsMap()
for i in CS_datat:
    CS_data[i] = []
    for j in CS_datat[i]:
        CS_data[i].append(j[0])

#for i in CS_data:
#    CS_data[i].extend(cs_da[i])

#generate CS summarization
cs = dis2.filter(lambda x:len(x[1])>1).map(lambda x:compt_stat2(x))
...
for i in DS_data:
    aln += len(DS_data[i])
#print(aln)
intermediate.append((len(path_list), len(DS_data.keys())-1, aln, 0,0,0))
print(intermediate)
...
o1 = sys.argv[3]
o2 = sys.argv[4]
with open(o1,'w') as f_j:
    f_j.write(outputj)

# inter_dict = {'round_id':[],'nof_cluster_discard':[],'nof_point_discard':[],'nof_cluster_compression':[],'nof_point_compression':[],'nof_point_retained':[]}
# for i in intermediate:
#     inter_dict['round_id'].append(i[0])
#     inter_dict['nof_cluster_discard'].append(i[1])
#     inter_dict['nof_point_discard'].append(i[2])
#     inter_dict['nof_cluster_compression'].append(i[3])
#     inter_dict['nof_point_compression'].append(i[4])
#     inter_dict['nof_point_retained'].append(i[5])
with open(o2,'w') as f:
    f.writelines('round_id,nof_cluster_discard,nof_point_discard,nof_cluster_compression,nof_point_compression,nof_point_retained')
    f.writelines('\n')
    for i in intermediate:
        f.writelines(str(i).replace('(','').replace(')','').replace(', ',','))
        f.writelines('\n')
#print(2)
