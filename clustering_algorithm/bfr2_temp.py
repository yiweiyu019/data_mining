...
def fit1(x, y, z):
    ...
    center_info = {}
    cluster = {}
    for i, j in enumerate(random.sample(x.keys(), y)):
        center_info.setdefault(i, x.get(j))
        cluster.setdefault(i, list())
    count = 1
    while True:
        for key in list(x.keys()):
            temp = {}
            for k in center_info.keys():
                temp[(k, key)] = compute5(center_info[k], x[key])
            m1 = list(sorted(temp.items(), key=lambda x: x[1]))[:1]
            cluster[m1[0][0][0]].append((m1[0][0][1], x[m1[0][0][1]]))
        for kk, pp in cluster.items():
            tt = []
            tt.append(center_info.get(kk))
            for p1 in pp:
                tt.append(x.get(p1[0]))
            center_info[kk] = [sum(i) / len(i) for i in zip(*tt)]
        if count >= z:
            break
        cluster.clear
        count += 1
...
def compute5(x, y):
    return float(math.sqrt(sum([(a - b) ** 2 for (a, b) in zip(x, y)])))
...
sample_data = data.filter(lambda x: x[0] < 10000).collectAsMap()
temp2 = fit1(sample_data, n_cluster, 5)
sample_cluster = sc.parallelize([(k, v) for k, v in temp2.items()])
...
temp3 = data.collectAsMap()
temp4 = fit1(temp3, 3 * n_cluster, 3)
dis2 = sc.parallelize([(k, v) for k, v in temp4.items()])
...
for pp in range(1, len(path_list)):
    ...
    ma_dis = data_2.map(lambda x: compute_maha(x, summarization)).groupByKey().mapValues(
        list)  # remaining -->ds+(points)
