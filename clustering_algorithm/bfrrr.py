...
def clustering(x,y):
    ...
    for i in range(0,5):
        result = {}
        for element in x:
            cluster_id,datac = computet(element,initial)
            if cluster_id in result:
                result[str(cluster_id)].append(datac)
            else:
                result[str(cluster_id)] = [datac]
        result_sum = {}
        for i in result:
            result_sum[i] = compt_stat3(result[i])
        initial = []
        for i in result_sum:
            initial.append((i,result_sum[i][0]))
    data_result = {}
...
if size>20000:
   sampleall = data.takeSample(False,20000)
else:
...
#dis = [('4',[rid1,rid2...]),...]
ds_da,summarization = clustering(sampleall,central)
# for i in range(0,5):
